...
average_size_mean_by_valence = (
    merged_data_2020.groupby('valence')['size_mean']
    .mean()
    .fillna(0)
    .to_dict()
)
...
for label, valence_value, color in valence_scenarios:
    mean_size_mean = average_size_mean_by_valence.get(valence_value, 0)
    print("average size of event: " + str(mean_size_mean) + " valence_value: " + str(valence_value))
     
    # mean_size_mean = merged_data_2020['size_mean'].mean()  # testing with a fixed value

    print("mean_size_mean: " + str(mean_size_mean))
    future_exog = [[1, valence_value * mean_size_mean * .05]] * forecast_steps
